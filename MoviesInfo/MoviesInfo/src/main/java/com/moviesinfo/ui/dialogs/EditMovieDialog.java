...
public class EditMovieDialog extends JDialog {
    private JTextField nameField, directorField, actorsField, runningTimeField;
    private JTextArea reviewArea, commentsArea;
    private JComboBox<String> yearBox;
    private JPanel genresPanel;
    private JSpinner imdbScoreSpinner;
    private DatabaseManager dbManager;
    private Runnable onMovieEdited;
    private int movieId;

    public EditMovieDialog(Window owner, DatabaseManager dbManager, Object[] movieRow, Runnable onMovieEdited, int movieId) {
        super(owner, "Edit movie", ModalityType.APPLICATION_MODAL);
        this.dbManager = dbManager;
        this.onMovieEdited = onMovieEdited;
        ...
        c.gridx = 1; imdbScoreSpinner = new JSpinner(new SpinnerNumberModel(5.0, 0.0, 10.0, 0.1));
        ((JSpinner.DefaultEditor)imdbScoreSpinner.getEditor()).getTextField().setColumns(3);
        form.add(imdbScoreSpinner, c);
        ...
        // Fill fields with movieRow data
        nameField.setText((String)movieRow[0]);
        imdbScoreSpinner.setValue(movieRow[1]);
        String[] selectedGenres = ((String)movieRow[2]).split(", ");
        for (Component comp : genresPanel.getComponents()) {
            if (comp instanceof JCheckBox) {
                JCheckBox cb = (JCheckBox)comp;
                for (String g : selectedGenres) {
                    if (cb.getText().equals(g)) cb.setSelected(true);
                }
            }
        }
        yearBox.setSelectedItem(movieRow[3].toString());
        directorField.setText((String)movieRow[4]);
        actorsField.setText((String)movieRow[5]);
        reviewArea.setText((String)movieRow[6]);
        commentsArea.setText((String)movieRow[7]);
        String runningTimeVal = (movieRow[8] != null && !movieRow[8].toString().trim().isEmpty()) ? movieRow[8].toString() : "";
        if (runningTimeVal.endsWith("minutes")) {
            runningTimeVal = runningTimeVal.replace("minutes", "").trim();
        }
        runningTimeField.setText(runningTimeVal);

        editBtn.addActionListener(e -> {
            try {
                String name = nameField.getText().trim();
                double imdbScore = ((Number)imdbScoreSpinner.getValue()).doubleValue();
                StringBuilder genresStr = new StringBuilder();
                for (Component comp : genresPanel.getComponents())
                    if (comp instanceof JCheckBox && ((JCheckBox)comp).isSelected())
                        genresStr.append(((JCheckBox)comp).getText()).append(", ");
                String genresFinal = genresStr.toString();
                if (genresFinal.endsWith(", ")) genresFinal = genresFinal.substring(0, genresFinal.length()-2);
                ...
                PreparedStatement pstmt = dbManager.getConnection().prepareStatement(
                    "UPDATE movies SET title=?, imdb_score=?, genre=?, year=?, director=?, actors=?, review=?, comments=?, running_time=? WHERE id=?"
                );
                pstmt.setString(1, name);
                pstmt.setDouble(2, imdbScore);
                pstmt.setString(3, genresFinal);
                ...
                pstmt.executeUpdate();
                if (onMovieEdited != null) onMovieEdited.run();
                dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error editing movie: " + ex.getMessage());
            }
        });
